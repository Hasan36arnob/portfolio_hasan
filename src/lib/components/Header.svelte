		...
		>
			<div class="flex items-center justify-between">
				<a
					href="/"
					aria-label="Homepage"
					class="text-xl font-extrabold tracking-normal text-slate-900"
					>{cv_data.name.first} {cv_data.name.last}</a
				>
				<button
					aria-expanded={open}
			...
			>
				...
				{#if isFilled.link(settings.data.cta_link)}
					<Button linkField={{ url: `/contact`, link_type: 'Web' }} label="Contact" />
				{/if}
			</ul>
			...
			<ul class="relative z-50 hidden flex-row items-center gap-1 bg-transparent py-0 md:flex">
				...
				{/each}
				{#if isFilled.link(settings.data.cta_link)}
					<Button linkField={{ url: `/contact`, link_type: 'Web' }} label="Contact" class="ml-3" />
				{/if}
			</ul>
		</div>
	</nav>
