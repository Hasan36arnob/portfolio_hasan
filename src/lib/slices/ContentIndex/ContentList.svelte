<script lang="ts">
	...
	$: contentImages = items.map((item) => {
		if (item.id && item.id.startsWith('user-')) return null;
		const image = item.data?.hover_image && isFilled.image(item.data.hover_image) ? item.data.hover_image : fallbackItemImage;
	...
	const handleMouseMove = (e: MouseEvent) => {
		if (currentIndex !== undefined && items[currentIndex]?.id?.startsWith('user-')) {
			gsap.to('.hover-reveal', { opacity: 0, visibility: 'hidden', duration: 0.3 });
			return;
		}
...
<ul on:mouseleave={onMouseLeave} class="grid border-b border-b-slate-100">
	{#each items as post, index (post.id + index)}
		...
		>
			<PrismicLink
				document={post.id.startsWith('user-') ? undefined : post}
				url={post.id.startsWith('user-') ? post.url : undefined}
					...
					{#if post.id.startsWith('user-')}
						<div class="mt-4 flex gap-6">
							<a
								...
								target="_blank"
								rel="noreferrer"
								class="text-xl font-bold text-slate-100 underline decoration-yellow-500 underline-offset-4 hover:text-yellow-400"
							>
								Live Demo
							</a>
							{#if post.source_code}
